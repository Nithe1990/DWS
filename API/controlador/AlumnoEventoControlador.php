<?
    class AlumnoEventoControlador extends ControladorPadre{
        ...
        public function consultar(){
            ...
            if(count($recurso) == 3){
                $alumnoEvento = AlumnoEventoDAO::findById($recurso[2]);
                $data = json_encode($alumnoEvento);
                self::respuesta($data, array('Content-Type: application/json', 'HTTP/1.1 200 OK'));
            }
        }
        ...
        public function crear(){
            ...
            if(isset($dato['idAlumno']) && isset($dato['idEvento'])){
                $alumnoEvento = new AlumnoEvento($dato['idAlumno'], $dato['idEvento']);
                if(AlumnoEventoDAO::insert($alumnoEvento)){
        ...
        public function modificar(){
            ...
            if(count($recurso) == 3){
                ...
                if(isset($dato['idAlumno']) && isset($dato['idEvento'])){
                    $alumnoEvento = new AlumnoEvento($dato['idAlumno'], $dato['idEvento']);
                    $alumnoEvento->idAlumnoEvento = $recurso[2];
                    if(AlumnoEventoDAO::update($alumnoEvento)){
                    ...
                    self::respuesta('', array('HTTP/1.1 400 El recurso está mal formado /alumno_evento/{id}'));
                }
            }
        }
        ...
        public function borrar(){
            $recurso = self::comprobarRecurso();
            if(count($recurso) == 3){
                if(!AlumnoEventoDAO::delete($recurso[2])){
